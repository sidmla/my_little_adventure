...
const Logo = () => (
  <Link to="/" data-testid="logo-link" className="flex items-center gap-2.5 group">
    <img
      ...
      className="w-12 h-12 sm:w-14 sm:h-14 rounded-full object-cover shrink-0 ring-2 ring-[var(--mla-yellow-bright)] shadow-md shadow-black/10 group-hover:shadow-lg transition-all"
    />
...
function TabDropdown({ item }) {
  ...
  const groups = navGroups(item.key);
  const totalTrips = groups.reduce((n, [, t]) => n + t.length, 0);
                  ...
                  <div key={item.key}>
                    ...
                    {isOpen && (
                      <div className="pl-4 flex flex-col gap-1 pb-2">
                        {navGroups(item.key).map(([groupLabel, gTrips]) => (
