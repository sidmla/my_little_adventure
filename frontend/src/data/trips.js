...
// Labels for the category groups shown as section headings inside dropdowns.
...
// Order categories appear in within a dropdown.
const CATEGORY_GROUP_ORDER = ["international", "group", "couple", "pilgrimage"];

// Generic grouping for any nav type's dropdown.
// Domestic groups by state; every other tab groups by trip category.
// Returns [ [groupLabel, trips[]], ... ]
export function navGroups(type) {
  if (type === "domestic") return domesticByState();
  const trips = TRIPS.filter((t) => (t.tripTypes || []).includes(type));
  const groups = {};
  trips.forEach((t) => {
    const k = t.categoryKey || "other";
    (groups[k] = groups[k] || []).push(t);
  });
  const ordered = [];
  CATEGORY_GROUP_ORDER.forEach((k) => {
    if (groups[k]) {
      ordered.push([CATEGORY_GROUP_LABELS[k] || k, groups[k]]);
      delete groups[k];
    }
  });
